...
> [!IMPORTANT]
> Документация описывает фактическую логику переданных DAG-ов. Она не заменяет миграции БД, инфраструктурный код и секреты окружения: этих файлов в исходном комплекте нет.
...
> [!WARNING]
> Текущие cron-выражения **не реализуют требуемый порядок одной цепочки**: кампанийная и детальная витрины запускаются в `22:20` и `22:40`, а матчинг CRM — в `01:15`. Кроме того, timezone не задан внутри DAG-ов. До эксплуатации IT должен либо выровнять cron, либо связать DAG-и через Airflow Datasets, сенсоры или управляющий DAG.
...
```text
OAuth-токены Яндекс Директа
пароли и строки подключения к БД
экспорт реальных Airflow Variables
дампы CRM с персональными данными
логи с токенами или персональными данными
```

Для production предпочтительно хранить секреты в Airflow Connections/Secrets Backend, а не в открытом JSON внутри репозитория.

## Статус комплекта

- Пять исходных DAG-файлов перенесены без изменения кода; нормализованы только два имени файлов.
- Все пять файлов проходят проверку синтаксиса Python через `py_compile`.
- DDL внешних таблиц, инфраструктурные манифесты, версии зависимостей и CI/CD в переданном комплекте отсутствуют.
